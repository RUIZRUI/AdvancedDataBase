...
if( isset($_REQUEST['empnum']) && !empty($_REQUEST['empnum']))
{
	$empnum = $_REQUEST['empnum'];
}

$chain .= "empnum=$empnum<br>\n";


// $curs1 = OCI_parse($connection, "SELECT part_id, part_name FROM part where part_id = '$partid'");
$curs1 = OCI_parse($connection, "select po.po_number as a, po.po_date as b, po.total as c, po.status as d from pa_task pt, purchase_order po where po.po_number=pt.po_number and pt.emp_num='$empnum'");
...
OCI_Define_By_Name($curs1,"C",$total);
OCI_Define_By_Name($curs1,"D",$status);

/*      4. Execution of the SQL request with an immediate commit to free locks */
OCI_Execute($curs1, OCI_COMMIT_ON_SUCCESS);
$chain .= "<b>po_number  po_date total status</b><br>\n";
